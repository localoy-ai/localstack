...
**The brain widens the dedupe set beyond this repo.** Read the shipped
...
turn). Cut any candidate whose canonical domain appears in a record name
(`lead-<domain>`), logged as "cut: in the brain — <record name>". This
catches leads shipped from OTHER repos and machines that `shipped/*.csv`
cannot see. Neither door available → skip silently; the file scan above
still stands. Never let a brain error kill the report — note it and
continue.
