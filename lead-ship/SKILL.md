...
- **Qualified list (required):** `ls reviews/*.csv 2>/dev/null | sort -rV | head -1`,
...
- `shipped/{YYYY-MM-DD}-{slug}.csv` — the reviews CSV columns plus
  ...
  Never overwrite an existing artifact: if `shipped/{date}-{slug}.csv` already exists, append a sequence suffix before the extension — `shipped/{date}-{slug}-2.csv`, then `-3`… (count the existing matches and add one). The summary file takes the
  same suffix.
...
localbrain put "lead-<canonical-domain>" --type lead \
  --field domain=<canonical-domain> --field shipped=<YYYY-MM-DD> \
  --body "<Company Name> — <canonical domain>. Shipped <YYYY-MM-DD> in <shipped csv filename>. Contact: <Decision Maker Name>, <Title>."
```

The two `--field` pairs make the record structurally queryable
(`localbrain list --where domain=<domain>`), so dedupe never depends on
parsing prose. Requires localbrain >= 0.2.0; on an older CLI the flags
error — drop them and note "localbrain pre-0.2.0 — fields omitted" in
the summary rather than failing the ship.
...
`.brain/put/lead-<canonical-domain>.md` per shipped row, with a
frontmatter fence carrying the same structure, then the body verbatim:
...
The daemon stores them after the turn, fields included. Neither the CLI nor `.brain/` → skip and add one line to the
summary's Known gaps: "localbrain unavailable — this shipment is invisible
to cross-repo dedupe (run /setup-localbrain)". A brain error never blocks
the package — the CSV already shipped; note the error and move on. The
files in `shipped/` stay the source of truth; the brain is the index other
repos can reach.
