...
```
ls briefs/*.md   2>/dev/null | sort -rV | head -1
...
(Newest = the `{date}` filename prefix; `-V` ranks a `-2` rerun suffix
above its base file. Never sort by mtime.)
...
Never overwrite an existing artifact: if `retros/{date}-{slug}.md` already exists, append a sequence suffix before the extension — `retros/{date}-{slug}-2.md`, then `-3`… (count the existing matches and add one).
Template:
